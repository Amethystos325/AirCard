...
async def native_airtraffic(runtime: Path, udid: str, assets: Path | None = None, *, check_sync: bool = False) -> dict:
    command = [sys.executable, str(ROOT / "backend" / "windows_airtraffic.py"),
...
def trace_text(data: bytes) -> str | None:
    # Same bounds and offsets as frontend/swift/helpers/os_trace.h; multiline messages survive.
...
async def device_probe(args) -> dict:
    ...
    async with await select_device(args.udid) as client:
        ...
        if args.command == "device":
            ...
            from backend.windows_canary import protected_canary
            runtime = args.apple_runtime or prepare_apple_runtime()
...
def main(argv=None) -> int:
    ...
    try:
        ...
        report["nextStep"] = {
            ...
            "ModuleNotFoundError": "Run backend/run_windows_probe.ps1 -Setup with Python 3.12.",
