...
Tauri 2、React、TypeScript 和独立 Python 3.12 后端。SwiftUI 应用及其构建脚本位于 `../swift/`。
...
桌面界面参照 `../swift/swiftui/AirCardApp.swift`：62px 紧凑工具栏、设备状态胶囊、扫描提示条、290 × 182 卡面、自适应网格、卡片内读取 / 更换 / 导出按钮，以及底部状态和折叠日志。CSS 复现半透明材质、卡面光泽、悬停倾斜和读取扫光；减少动态效果的系统设置会关闭动画。系统原生毛玻璃和字体渲染在不同平台仍有差异。
