...
const args = process.platform === 'darwin'
  ? ['-m', 'PyInstaller', ...output, join(root, 'backend/aircard-backend.macos.spec')]
  : ['-m', 'PyInstaller', ...output, '--name', 'aircard-backend', '--onedir', '--specpath', join(root, 'build'), '--paths', root, '--hidden-import', 'pymobiledevice3.services.afc', '--hidden-import', 'pymobiledevice3.lockdown', '--hidden-import', 'pymobiledevice3.usbmux', '--hidden-import', 'pefile', join(root, 'backend/desktop_backend.py')];
...
if (target !== join(root, 'frontend', 'cross-platform', 'src-tauri', 'binaries', 'backend')) throw new Error('Invalid packaging destination');
