...
## SwiftUI 功能迁移检查（2026-09-24）

- SwiftUI v0.2 保留原有顶栏、卡片网格和底部日志布局，新增原生卡面编辑弹层、首次备份恢复、待恢复提示、取消操作、设备归属过滤、会话隐藏、语言和外观菜单。
- Swift 编译、arm64 应用与 DMG 构建通过；应用签名和 DMG 校验通过。冻结后端的 `hello` / `shutdown` 协议通过。
- 后端 72 项测试完成，71 项通过、1 项按环境跳过。跨进程事务锁加入测试，避免 SwiftUI 和 Tauri 同时执行卡片事务。
- 本机启动后，检查了原布局、已缓存卡面在离线编辑弹层的显示、选图只进入预览、生成裁剪文件、临时隐藏及恢复、设置菜单。没有执行设备写入。
- SwiftUI 目前使用双端版固定的三种 combined artwork 文件备份和导出。旧 Swift 的 manifest 范围导出未接到新事务界面。Mac 真机读取、写入、恢复与不同系统版本仍需验证。
